...
function init_quickpay_gateway() {
	...
	class WC_Quickpay extends WC_Payment_Gateway
	{
		...
		*/
		public function hooks_and_filters() 
		{
			...
			if( is_admin() ) {
		    	...
		    	add_action( 'woocommerce_order_status_completed', array( $this, 'capture_on_order_status_complete' ) );  
			...
			if( isset( $_POST['quickpay_action'] ) AND isset( $_POST['post'] ) ) 
			{
				...
				if( in_array( $param_action, $allowed_actions ) ) 
				{
					if( $api->is_action_allowed( $param_action ) ) 
					{
						...
						$api->action_router( $order->id , $param_action, $extras );
					}
				}
		...
		*/
		...
		/**
		* capture_on_order_status_complete function.
		*
		...
		* @return void
		*/
		public function capture_on_order_status_complete() 
		{
		...
		*/	
		private function generate_quickpay_form( $order_id ) 
		{
			...
			$md5check = md5(
				...
				. $callbackurl . $autocapture . $cardtypelock . $splitcapture .  $description . $test_mode . $this->s( 'quickpay_md5secret' )
			);
